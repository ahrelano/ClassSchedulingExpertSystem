...
if (isset($list_teachers)) {
	...
	echo '<h3>Search teacher schedule</h3>';
	if (isset($teachers)) {
		echo $this->Form->control('teachers', ['options'=>$teachers, 'onchange'=>'teacherSchedule(this.value)', 'empty'=>'Choose a teacher']);
	}
	$grade = [
		'7' => 'Grade 7',
        '8' => 'Grade 8',
	...
	echo '<h3>Search room schedule</h3>';
	if (isset($grade_junior) || isset($grade_senior)) {
		echo $this->Form->control('grade', ['options'=>$grade, 'onchange'=>'searchroom(this.value)', 'empty'=>'Grade']);
		echo '<div class="rooms"><select name="rooms" id="rooms"><option value="">Rooms</option></select></div>';
	}
}
